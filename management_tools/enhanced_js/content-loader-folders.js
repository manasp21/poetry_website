// js/content-loader.js - Enhanced for Folder Structure
// Automatic image detection and folder-based poem loading
...
async function checkImageExists(imagePath) {
    try {
        const response = await fetch(`/poetry_website/${imagePath}`, { method: 'HEAD' });
        return response.ok;
    } catch (error) {
        return false;
    }
...
// Get available image formats for a poem folder
async function detectImageForPoem(poemPath) {
    const folderPath = poemPath.substring(0, poemPath.lastIndexOf('/'));
...
// Function to fetch and parse poems progressively with automatic image detection
async function fetchAllPoems(limit = null) {
    ...
    // Fallback to static poem list with folder structure
    const poemFilePaths = [
        "Poetry/1/poem.md",
        "Poetry/2/poem.md",
        "Poetry/3/poem.md",
    ...
    for (const filePath of pathsToProcess) {
        try {
            ...
            // Extract poem number from path (Poetry/X/poem.md -> X)
            const poemNumber = filePath.split('/')[1];
            const fileName = `poem_${poemNumber}`;

            // Automatically detect image
            console.log(`🔍 Detecting image for ${filePath}...`);
            const detectedImage = await detectImageForPoem(filePath);
            
            if (detectedImage) {
                console.log(`✅ Found image: ${detectedImage}`);
            } else {
                console.log(`❌ No image found for poem ${poemNumber}`);
            }
            ...
            const poemEntry = {
                ...
                poemNumber: poemNumber, // Store poem number for reference
                folderPath: filePath.substring(0, filePath.lastIndexOf('/')) // Store folder path
            };
    ...
    console.log(`📊 Loaded ${poemsData.length} poems from folder structure`);
    console.log(`🖼️  Images detected: ${poemsData.filter(p => p.image).length}/${poemsData.length}`);
...
console.log('📁 Content loader initialized for folder structure');
console.log('🖼️  Automatic image detection enabled');

// Added comment to indicate folder structure support